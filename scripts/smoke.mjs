...
// advance a section (Backdrop 01: Cladding, Ongoing → Finished; phase 56% → 67%)
const before = await page.textContent('.card .mono >> nth=0')
await page.click('button[aria-label="Advance Cladding"]')
await page.waitForTimeout(300)
console.log('SECTION ADVANCE:', before.trim(), '->', await page.textContent('.card .mono >> nth=0'))
// filter chip
...
await page.click('.stack-holder button:has-text("Share PDF")')
const download = await dl
console.log('PDF DOWNLOAD:', download.suggestedFilename())
await page.click('.stack-holder .back-chevron >> nth=-1')
...
await page.click('.stack-holder >> text=Open >> nth=0')
await page.waitForTimeout(1500)
